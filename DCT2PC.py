...
def PCPCA(SXtrain,SXtest):
    '''通过 PCA 方法提取训练集中的主要特征（主成分），并将训练集和测试集都投影到这些主成分上，实现数据降维，
    同时返回降维后的结果、主成分载荷和保留的主成分数量。'''
    [u, s, v]=np.linalg.svd(SXtrain,full_matrices=False) 
    sumvar=sum(s)
    sum1=0
...
if __name__ == '__main__':
    ...
    #KS_tran_num = 60 #for cal model
    KS_tran_nums = [10, 20, 30, 40, 50, 60]


     # datafile =u'./2003TN.mat'
    # dataset = loadmat(datafile)
    # MX = dataset['X2003']; Y = dataset['Y2003'];
    # MX = MX.T; Y = Y.T;
    # datafile =u'./2004TN.mat'
    # dataset = loadmat(datafile)
    # SX = dataset['X2004'];
    # SX = SX.T
    # KS_PLS_num = 201
    # KS_tran_nums = [10, 15, 20, 25, 30, 35, 40, 45, 50, 55,60]  
    RMSEP_ELM, RMSEP_PLS = [], []
    for i, KS_tran_num in enumerate(KS_tran_nums):
    ...
    #  PCELM for TRANSFER
        ...
        [TMX, vi, pc] = PCPCA (MXtrain1,SXtrain1)
        ...
        #SXtestpre = dot(SXtestpre1, vi)

        SXtestpre =  SXtestpre1 + np.mean( MXtrain, axis=0)
        ...
        print( "RMSEP_S_trans:", RMSEP_Stestpre, "Q2_S_trans:", Q2_Stestpre)
    ...
    #  ELM for Quantification
        ...
        print( "RMSEP_M_ELM:", RMSEP_testpre_Y, "Q2_M_ELM:", Q2_testpre_Y)
        print( "RMSEP_S_trans_ELM:", RMSEP_tran_Y, "Q2_S_trans_ELM:", Q2_tran_Y)
        
        RMSEP_ELM.append(RMSEP_tran_Y)
        RMSEP_PLS.append(RMSEP_Stestpre)
        
    print('PLS')
    for val in RMSEP_PLS:
        print(val)
    print('ELM')
    for val in RMSEP_ELM:
        print(val)
